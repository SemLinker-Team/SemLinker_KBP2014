...
public class NormalizeNE {
	 ...
	 */
	public  AnnotationInterface rerankNE(AnnotationInterface annotations ){
							...
							if (! refNE.contentEquals("")){
								...
								for(int z=0; z < annotations.size() ; z++){
									if (annotations.getMetadatakey(z) != null){
										if ( annotations.getMetadatakey(z).contains(referencemetadata) && !annotations.getNELabel(z).contains("QUT")){
											...
											for (int w = 0; w < 7; w++){
													annotations.setNElabel(z + w, refNE);
													int pos = z + w + 1;
