...
import kbp2014.defineLink.LinkQueries;
...
import kbp2014.tools.ManageQueries;
 ...
 *
 */
public class LinkEntitiesQueries {
...
//HA            	if (text.length() > 1) {            	
           	if (text.contains("<query")) {
                ...
                query = querymanager.extractQueryData(reader, querymanager.extractQueryId(text), nbQueries);
                    ...
                    /**
                     * Grabbing the original content for offsets(HA)
                     */
...
//HA           		content  = content.replace(query.originalQuery , query.name);
...
// HA                  	if ( content.length() > 60000){
       		      ...
       		      //   		int lenghtofdocresized = 5000;
                		...
                		//content = documentNormalizer.NormalizeCapSequences(content, query.name, logHeadlines); // always first (try to) remove caps in sentences   
...
//                      annotHandler.handleForumAuthors(annotations);
                        ...
                        /** Comment / Uncomment to apply/not apply mutual desambiguation **/
                        ...
                        /** Comment / Uncomment to apply/not apply NE normalizer **/
                        // apply NE normalizer
                        annotations = NEnorm.rerankNE(annotations);
                   ...
                   int linkAtPos = RetrieveExactMentionAnnotation.getAnnotationAtPosition(annotations, query.name, query.beg, query.end, originalContent, logging);
                        ...
                        // search match
                        //--------------------------------
                    LinkQueries link = new LinkQueries(annotations, query.normalizedName, query.name, linkAtPos, KBCorrespondanceTable, logging, maxnumberofannotations);
