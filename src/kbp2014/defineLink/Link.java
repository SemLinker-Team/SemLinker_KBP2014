...
public class Link {
	 ...
	 */
	public Link(AnnotationInterface annotations, 
	...
	{	
        ...
        decideLink(annotations, normalizedQueryName, queryName, queryPosition, linkAtPos, log, maxnumberofannotation);		
	 ...
	 * @return
	 */
	private void decideLink(AnnotationInterface annotations, String QnameNormalized, String Qname,  int queryPosition, int linkAtPos, Logging log, int maxnumberofannotation){
		...
		String MentionExpanded = QnameNormalized;	
	...
	//			charlenght = charlenght + annotations.getWordAtPos(h).length() +1;
				...
				String StrToCompare = annotations.getSurfaceFormNormalizedAtPos(linkAtPos);				
				String NELabelToEval = annotations.getNELabel(linkAtPos);
				// sort found references
				if (  StrToCompare != null && !NELabelToEval.contains("PER.DFA")){					
						
								String[] kbkey = new String[maxnumberofannotation];
								for (int z= 0; z < maxnumberofannotation; z++){
									if ( (annotations.getMetadataKeyRanked(z, linkAtPos) != null) ) {									
										kbkey[z] = KBCorrespondanceTable.returnKbpKey(annotations.getMetadataKeyRanked(z, linkAtPos));
									}else{
										kbkey[z] = null;
									}
								}
								...
								results[StringArrayIdx][1]=annotations.getSurfaceFormNormalizedAtPos(linkAtPos).toLowerCase(); // Surface form of the mention -> normalized						
								results[StringArrayIdx][2]=annotations.getMetadata(linkAtPos); // Save the Wikimeta key than can disambiguate event without KB key
								results[StringArrayIdx][3]=annotations.getNELabel(linkAtPos); // Save the Wikimeta EN
								...
								if (setametalink && annotations.getMetadatakey(linkAtPos).contains("NIL") ){							
									...
									// display the results
									//-------------------------							
									System.out.print("    --" + linkAtPos + ":" + charlenght + ":" + annotations.getMetadata(linkAtPos) + ":" + annotations.getMetadatakey(linkAtPos) + ":" + annotations.getNELabel(linkAtPos)  + 
											"   (mention:"+ annotations.getSurfaceFormNormalizedAtPos(linkAtPos)+")   " + 
											"   { "
									);
									...
									for (int o = 0; o< maxnumberofannotation; o++){
										System.out.print(annotations.getMetadataKeyRanked(o, linkAtPos) + "/");
								...
								if (annotations.getMetadatakey(linkAtPos) !=null && !annotations.getMetadatakey(linkAtPos).contains("NIL") ) setametalink = true;
						//	}
		...
		if (StringArrayIdx == 0){
			...
			//changing default UNK NE label to original label 
			//in the annotation object, because tab file was
			//outputting UNK labels to relevant entities (LOC, PER...)
		...
		if ( linkAtPos > 0 && annotations.getNELabel(linkAtPos).contains("LOC") )
		{
			...
			if (maxreferencedkey.contentEquals(referenceKey)){
				...
				if(annotations.getSurfaceFormNormalizedAtPos(linkAtPos)!=null)
					...
					allSf = annotations.getWordAtPos(linkAtPos);
			}
		}
	...
	public String bestEN(){
		...
		if(allNE.contains("LOC")){
			allNE = allNE.replace("LOC", "GPE");
		}
		
		return allNE.substring(0, 3);
	}
