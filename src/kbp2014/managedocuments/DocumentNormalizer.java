...
public class DocumentNormalizer {
    ...
    public static String RemoveTags(String content) {
        ...
        content = content.replaceAll("<DOC id=[^>]+>", ""); // remove Doc id tag
       	...
       	//handling unwanted chars in POSTER tags
        while(content.contains("<POSTER>")){  
        	String punct1 = ". . ."; String punct2 = "...";
        	String poster = StringUtils.substringBetween(content, "<POSTER>", "</POSTER>");
        	String newPoster = poster;
        	content = content.replaceFirst("<POSTER>", "Poster"); // change        	      	        	
        	//if(poster.contains("\"")) newPoster = poster.replace("\"", " ");
        	if(poster.contains("@")) newPoster = newPoster.substring(0, poster.indexOf("@"));        	
        	if(poster.contains(punct1)) newPoster = newPoster.replace(punct1, "");
        	if(poster.contains(punct2)) newPoster = newPoster.replace(punct2, "");
        	content = content.replace(poster, newPoster);
        }
        
        // on ng files
        //As we need to fetch the poster authors, change this condition:
        //content = content.replaceAll("<POSTER>[^>]+</POSTER>", ""); // remove poster name
       // content = content.replaceAll("<POSTER>", " Poster "); // change
        content = content.replaceAll("</POSTER>", " !Poster "); // change
        content = content.replaceAll("<QUOTE PREVIOUSPOST=\"[^>]+\">", ""); 
        content = content.replaceAll("<DOCID>[^>]+</DOCID>", ""); // remove DOCID definition
        //adding further replacements to send a cleaner content to the annotator
        content = content.replaceAll("<doc id=\"[^>]+\">", "");
        content = content.replaceAll("<DOC id=\"[^>]+\">", "");
        ...
        // NOTE: why is opening markup replaced by .? (MG)
        Lexicon lexicon;
        content = content.replaceAll("UTC\\s*</DATELINE>", " . ");
    ...
    public static String PrepareDocForMentionLocation(String originalText) {
    	...
    	//commenting this to avoid replacing the asterisk.
     ...
     * 
     */
    public String NormalizeCapSequences(String originalContent, Logging log) {
