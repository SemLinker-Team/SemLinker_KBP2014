...
import semkit.annotators.wikimetaAPICaller;
import kbp2014.managedocuments.SpellingCorrector;
import kbp2014.managedocuments.checkSuggestion;
			...
			for (int x=0; x < args.length; x++){
						...
						try{
								...
								if ( args[x].matches("-pathofxml") ){
									   // usually "~/workspace/SemLinker/resources/kbp2014/entitylinkingeval/tac_2013_kbp_english_entity_linking_evaluation_queries.xml";
		 ...
		 try {
	            ...
	            while ((ligne = reader.readLine()) != null) {
	            	...
	            	if (ligne.contains("<div class=\"searchdidyoumean\">")){
	            		 ...
	            		 /**
	            		  * Handling new tags in wikipedia suggestion (HA)
	            		  */          		
	            		 
	            		 ligne = ligne.replaceAll("Results ", "");
	            		 ligne = ligne.replaceAll(" for ", "");
	            		 ligne = ligne.replaceAll(" of ", "");
	            		 ligne = ligne.replaceAll("(?<=<b>).*?(?=</b>)", "");
	            		 ligne = ligne.replaceAll("<b>", "");
	            		 ligne = ligne.replaceAll("</b>", "");
	            		 ligne = ligne.replaceAll("</div>", "");
	            		 String mention = ligne;
	            		 System.out.println("Sequence to Search: " + mention);
	            		 return mention;
