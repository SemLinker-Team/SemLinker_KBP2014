package kbp2014.managelinkoutput;
...
//		String[][] toCluster = new String[3000][7]; // query - KBLink - qname - expanded qname - mentionExpanded - URI or NORDF|NIL - NE
		String[][] toCluster = new String[15000][7]; // query - KBLink - qname - expanded qname - mentionExpanded - URI or NORDF|NIL - NE
		int nilClusterIdx = 0;
			...
			if ( toCluster[x][1].matches("E[0-9]+") && verifySizeOfMention.length > 1 ){
				...
				for (int y= x + 1 ; y< idx; y++){
					...
					if ( toCluster[x][4].contentEquals(toCluster[y][4]) && toCluster[x][1].matches("NIL")  ){
						...
						System.out.println("                Reattaching Query to a KB: " + toCluster[y][0] + " "  + toCluster[y][4]);
		...
		for (int x= 0 ; x< idx; x++){
			...
			//------------------------------------------
			// ***B) New - changing condition to accept clustering of single tokens as well!
			
			if (toCluster[x][1].contentEquals("NIL")){
				...
				for (int y= x + 1 ; y< idx; y++){
					...
					//changing for substring of surface form, to cluster "michael jackson" with "jackson"									
					if (toCluster[x][3].contains(toCluster[y][3]) && toCluster[y][1].contentEquals("NIL") ){						
						System.out.println("     Cluster by Surface form:" + toCluster[y][0] + " " + toCluster[y][5] + "         sf:" + toCluster[y][3] + "         key:" + toCluster[x][3]);
						// replace NIL by cluster Number
						toCluster[y][1] = NilNumber;						
		...
		try {
			...
			for (int x= 0 ; x< idx; x++){
				...
				writer.append(toCluster[x][0] + "\t" + toCluster[x][1]+ "\t" + toCluster[x][6] + "\t1.0" + "\n");
				writer.flush();
			}
