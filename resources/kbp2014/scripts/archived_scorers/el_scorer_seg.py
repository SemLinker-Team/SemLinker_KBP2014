...
# LJL: function that decides if a line of the gold standard file should be considered for the evaluation
# the function returns True if the line should be discarded and False when the line is retained for the evaluation
def isLineEscaped(lineCnt, selectedType):
    outFlag = True # by default the line is discarded
    d = re.split("\s+", lineCnt.strip())
    if len(d)<2:
        return True
    mention = d[0]
    kb_id   = d[1]
    ...
    #handle NILs
    if selectedType == "NIL":
        if kb_id.startswith("NIL") == True:
            outFlag = False
    #handle non-nils
    if selectedType == "NONIL":
        if kb_id.startswith("NIL") == False:
            outFlag = False
    #handle NW|WB|DF
    if selectedType in ["NW","WB","DF"]:
        if nwField == selectedType:
            outFlag = False
    #handle ORG, PER, GPE entities
    if selectedType == nerType:
        outFlag = False
    return outFlag

def readLinking(goldStdFile, enType=None):
    """
    ...
    for line in open(goldStdFile):
        ...
        if(len(d)==3):  # a system linking
         ...
         #--- updates LJL to only consider a specific type for the evaluation        
        if len(d) > 3:
            targetType = d[2]
            nwField = d[3]
        else:
        ...
        if enType != None:
            #function that decides according to the type of entity whether the line should be considered for the evaluation
            skipLine = isLineEscaped(line, enType)
            if (skipLine == True):
                continue
...
def systemsRankingScript(goldStdFile, systemsDir, focusElFile, targetType=None):

    gold_standard, gold_el2kbid = readLinking(goldStdFile)
#    gold_el2kbid = getMap(gold_standard)
    gold_els = frozenset(gold_el2kbid.keys())

    focus_linking, focus_el2kbid = readLinking(focusElFile, targetType)
...
if __name__ == "__main__":
    ...
    #-- variable to restrict the evaluation to some entity types
    _type = None

    if(len(sys.argv) != 3 and len(sys.argv) != 4):
        print "----------------------------------------"
        print "KBP2013 Entity Linking evaluation script"
        print "----------------------------------------"
        ...
        print " - --NE_TYPE specific named entity type for the evaluation values shall be in {--ORG|--GPE|--PER|--NIL|--NONIL|--NW|--WB|--DF}"
        print " Please note that 'NIL' element identifiers must follow the format NILXXX, where XXX is a string that doesn't include white space."
    else:
        goldStdFile = sys.argv[1]
        systemsDir = sys.argv[2]
        if (len(sys.argv) == 4) and sys.argv[3].startswith("--") == False:
            focusElFile = sys.argv[3]
        else:
            if (len(sys.argv) > 3) :
                if (sys.argv[3]).startswith("--") == True:
                    _type = sys.argv[3].split("--")[1]
                    if _type not in ["ORG","GPE","PER","NIL","NONIL","NW","WB","DF"]:
                        sys.exit()
            focusElFile = goldStdFile
        systemsRankingScript(goldStdFile, systemsDir,focusElFile, targetType=_type)
